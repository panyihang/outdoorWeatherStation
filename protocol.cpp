...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_BRIGHT_STATE, DP_TYPE_ENUM},
  ...
  {DPID_ALTITUDE, DP_TYPE_VALUE},
  {DPID_IS_RAIN, DP_TYPE_BOOL},
  {DPID_IS_CHARGE, DP_TYPE_BOOL},
  {DPID_RSET, DP_TYPE_BOOL},
  {DPID_TEMP_0, DP_TYPE_STRING},
};
...
void all_data_update(void)
{
    ...
    mcu_dp_enum_update(DPID_BRIGHT_STATE,当前亮度检测状态); //枚举型数据上报;
    mcu_dp_value_update(DPID_TEMP_CURRENT,当前当前温度); //VALUE型数据上报;
    mcu_dp_value_update(DPID_HUMIDITY_VALUE,当前当前湿度); //VALUE型数据上报;
    mcu_dp_value_update(DPID_AIR_PRESSURE,当前气压); //VALUE型数据上报;
    mcu_dp_value_update(DPID_LIGHT_INTENSITY,当前光照强度); //VALUE型数据上报;
    mcu_dp_value_update(DPID_ALTITUDE,当前海拔); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_IS_RAIN,当前是否下雨); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_IS_CHARGE,当前是否在充电); //BOOL型数据上报;
    mcu_dp_bool_update(DPID_RSET,当前重置); //BOOL型数据上报;
    mcu_dp_string_update(DPID_TEMP_0,当前temp_0指针,当前temp_0数据长度); //STRING型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_rset_handle
功能描述 : 针对DPID_RSET的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_rset_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    ...
    rset = mcu_get_dp_download_bool(value,length);
    if(rset == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_RSET,rset);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_temp_0_handle
功能描述 : 针对DPID_TEMP_0的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_temp_0_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_string_update(DPID_TEMP_0,value, length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_RSET:
            //重置处理函数
            ret = dp_download_rset_handle(value,length);
        break;
        case DPID_TEMP_0:
            //temp_0处理函数
            ret = dp_download_temp_0_handle(value,length);
        break;
